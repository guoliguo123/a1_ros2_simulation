...
namespace unitree_legged_control 
{
    ...
    controller_interface::return_type UnitreeJointController::init(const std::string & controller_name)
    {
        ...
        if (!A1_node->get_parameter("joint", joint_name)){
            //RCLCPP_ERROR(rclcpp::get_logger("joint"), "No joint given in namespace: '%s')", A1_node->get_namespace().c_str());
            return controller_interface::return_type::ERROR;//false;
        }
        ...
        if (!urdf.initParamWithNodeHandle("robot_description", n)){
            RCLCPP_ERROR(rclcpp::get_logger("joint"),"Failed to parse urdf file");
            return controller_interface::return_type::ERROR;
        }
        joint_urdf = urdf.getJoint(joint_name);
        if (!joint_urdf){
            RCLCPP_ERROR(rclcpp::get_logger("joint"),"Could not find joint '%s' in urdf", joint_name.c_str());
            return controller_interface::return_type::ERROR;
        }
        ...
        sub_cmd = A1_node->create_subscription<a1_msgs::msg::MotorCmd>(
        ...
        controller_state_publisher_.reset(
            new realtime_tools::RealtimePublisher<a1_msgs::msg::MotorState>(n, name_space + "/state", 1));
